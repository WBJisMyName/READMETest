int do_SMART_buffer_dump(int nargs, char **argv)//Show SMART 512-byte buffer using CMD56
{
...
int show_SMART_info(int nargs, char **argv) //Show SMART info(ex: Speed class/ Erase count/ FW version, etc...)
{
	...
	is_transcend_card(data_buff, TYPE_SMART); //Transcend SD card only()
	return ret;
}

int show_Health_info(int nargs, char **argv)//Show card life
{
	int argCmd56 = 0x110005F9;
	char data_buff[SD_SMT_BLOCK_SIZE];
	int fd, ret;
